...
export const useWallet = () => {
  ...
  // Create an Interface object once
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      // 3. Fetch NFT balance (now step 3 instead of 4)
      // Use the pre-parsed contractInterface
      ...
      // 4. Update store (now step 4 instead of 5)
      setConnection(true, address);
      setBalance(balance);

      // 5. If balance > 0, fetch all token details (now step 5 instead of 6)
      if (balance > 0) {
        try {
  ...
  const disconnectWallet = async () => {
    // Removed: await supabase.auth.signOut(); // Sign out from Supabase
    reset();
