...
class Labyrinth {
  ...
  private bossSafeTiles = new Set<string>(); // Add this property to track safe tiles
  ...
  private _placeBossArea(floorMap: (LogicalRoom | 'wall')[][], floor: number, rooms: Room[]) {
    ...
    // Place the boss at the end of the passage
    const bossX = passageEndX;
    const bossY = passageCenterY;
    if (bossX >= 0 && bossX < this.MAP_WIDTH && bossY >= 0 && bossY < this.MAP_HEIGHT) {
      floorMap[bossY][bossX] = new LogicalRoom(
        `boss-room-${bossX}-${bossY}-f${floor}`,
        "The Watcher's Core",
        "A massive, pulsating eye floats in the center of this chamber, watching your every move."
      );
      this.bossLocation = `${bossX},${bossY},${floor}`;
    }
  }
    ...
    if (this.bossState === 'red_light' && isInBossPassage && !isOnSafeTile) {
      if (now - this.lastGazeDamageTimestamp > gazeDamageInterval) {
        ...
        if (this.playerHealth <= 0) {
          if (!this._tryActivateWellBlessing(this.playerName, this.elapsedTime, "The Watcher's Gaze")) {
            this.addMessage("The Labyrinth's Gaze consumes you. Darkness... Game Over.");
            this.setGameOver('defeat', this.playerName, this.elapsedTime, "The Watcher's Gaze");
            return;
          }
        }
      }
